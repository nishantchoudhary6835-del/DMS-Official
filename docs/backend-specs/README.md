...
| `AUTH_PASSWORD.md` | New 2026-08-18: Forgot Password (`POST /auth/forgot-password` → `POST /auth/verify-forgot-password-otp`, OTP + new password sent together in one call) and Change Password (`POST /auth/change-password`, requires old password). Both invalidate the existing refresh token on success — implemented client-side with an auto-relogin after Change Password so the current device doesn't get logged out too (see `useChangePassword.js`). Not yet exercised live against either backend |
...
| `SUPER_ADMIN_ROLE_MIDDLEWARE_EMPLOYEE_NOT_FOUND_BUG.md` | Super Admin gets 403 "Employee information not found." on every single call to `GET /user`, `/permission`, `/role-permission`, `/acl` — 100% reproducible, 9/9 calls this session. Root-caused to `role.middleware.js`'s `authorize()` depending on `req.employee`, which is only ever set in one place (`auth.middleware.js`) and apparently never survives to be readable there for this account, even though the identical `authenticate` middleware succeeds for `accessControl`-gated routes (`/employee`, `/department`) in the same session. Hard block, no frontend workaround exists |
